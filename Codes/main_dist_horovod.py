...
# from run_distributed import make_directory
...
cwd = os.getcwd()
model_loc = os.path.join(cwd, 'checkpoint')
...
def loss_score():
        ...
        opt = tf.train.AdamOptimizer(learning_rate=lr*hvd.size())
        opt = hvd.DistributedOptimizer(opt)
        classify_gradients = opt.compute_gradients(b_loss)
        _capped_gradients = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in classify_gradients if
                            grad is not None]
        train_step = opt.apply_gradients(_capped_gradients, global_step= global_step)
        # train_step = opt.minimize(l_loss)
...
train_file = "trainfile"+str(hvd.size())+"w"+str(hvd.rank())+".pkl"
val_file = "valfile"+str(hvd.size())+"w"+str(hvd.rank())+".pkl"
...
checkpoint_dir = model_loc if hvd.rank() == 0 else None
...
with tf.train.MonitoredTrainingSession(checkpoint_dir=checkpoint_dir, hooks=hooks, config=config) as mon_sess:
    ...
    while not mon_sess.should_stop() and epoch<EPOCHS:
        ...
        for batch in train_dset.get_batch(batch_size):
            ...
            curr_index = np.arange(hvd.rank(), len(batch), num_workers)
        ...
        logging.info ("loss: ", train_loss/count)
        logging.info ("avg score", 100*train_score/count)
        logging.info ("accuracy", 100*acc/count)
        ...
        if hvd.rank() == 0:
            ...
            logging.info("loss: ", val_loss / count)
            logging.info("avg score", 100 * val_score / count)
            logging.info ("accuracy", 100 * acc / count)
    ...
    f.close()
    end_time = time.time()
    logging.info (best_epoch, best_score)
