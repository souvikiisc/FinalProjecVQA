...
class VQAFeatureDataset():
    ...
    def get_batch(self, batch_size=256):
        ...
        for i, entry in enumerate(self.entries):
            ...
            question = np.array(entry['q_token'], dtype=np.int32)
            # entry['q_token'] = question
            features = self.features[entry['image']]
            answer = entry['answer']
            labels = np.array(answer['labels']).astype(int)
            scores = np.array(answer['scores'], dtype=np.float32)
            inp_len = entry['q_length']
            ...
            if len(labels):
                ...
                # l_batch.append(labels)
                # qlen_batch.append(inp_len)
                batch.append((question, features, target, inp_len))

            if count == batch_size or i == len(self.entries)-1:
                count = 0
                yield np.array(batch)
                batch = []

    def get_ans_dims(self):
        return self.num_ans_candidates
